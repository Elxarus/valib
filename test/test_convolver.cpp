...
double rms(Source *source, Filter *filter);
...
TEST(conv_zero, "Convolve with zero response")
  ...
  Noise noise_src(spk, size);
  ZeroSource zero_src(spk, size);
  noise_src.set_seed(seed);
...
TEST(conv_identity, "Convolve with identity response")
  ...
  Noise noise1(spk, size);
  Noise noise2(spk, size);
  noise1.set_seed(seed);
  noise2.set_seed(seed);
